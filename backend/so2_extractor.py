...
class SO2HuggingFaceAPI:
    def __init__(self, hf_token=None):
        """
        Initializes the connection to Hugging Face using DuckDB.
        """
        self.base_path = "hf://datasets/ObitUchiha91/Airlytics_data_set/SO2"
        ...
        try:
            ...
            print(f"[SO2Extractor] Failed to install/load httpfs extension: {e}")
        ...
        if token:
            try:
                ...
                print("[SO2Extractor] Successfully authenticated with Hugging Face.")
            except Exception as e:
                print(f"[SO2Extractor] Failed to create Hugging Face secret: {e}")
        else:
            print("[SO2Extractor] No HF_TOKEN provided. Dataset is public, so DuckDB should still work.")
    ...
    def get_state_summary(self, year="*"):
        ...
        query = f"""
            SELECT 
                ...
                AVG(so2_level) as mean_so2,
                MAX(so2_level) as max_so2
            FROM '{file_path}'
    ...
    def get_data_for_coordinate(self, lat, lon, year="2023"):
        ...
        if use_hf:
            try:
                ...
                if df.empty:
                    print(f"[SO2Extractor] Tight bbox empty, widening to 0.2 for ({lat}, {lon})")
                    df = self.get_by_bounding_box(lat - 0.2, lat + 0.2, lon - 0.2, lon + 0.2, year)
                
                if not df.empty:
                    print(f"[SO2Extractor] HF data: {len(df)} rows, columns: {list(df.columns)}")
                    ...
                    print(f"[SO2Extractor] âœ… SOURCE=HuggingFace for ({lat}, {lon}), "
                          f"closest={len(closest_coords)} rows, dist={min_dist:.6f}")
                    return closest_coords
                else:
                    print(f"[SO2Extractor] âš  HF returned empty for ({lat}, {lon}), year={year}")
            except Exception as e:
                print(f"[SO2Extractor] âŒ Hugging Face query failed: {e}. Falling back to local CSV.")
        ...
        try:
            ...
            if year == "*":
                file_pattern = os.path.join(csv_dir, "SO2_Data_Improv*.csv")
            else:
                file_pattern = os.path.join(csv_dir, f"SO2_Data_Improv{year}.csv")
            
            if not os.path.exists(csv_dir):
                print(f"[SO2Extractor] Local CSV directory not found at {csv_dir}")
                return pd.DataFrame()
            ...
            if not df.empty:
                ...
                closest_coords = df[df['dist'] == min_dist]
                closest_coords = closest_coords.drop(columns=['dist'])
                print(f"[SO2Extractor] Loaded closest coordinates from local CSV fallback for ({lat}, {lon})")
                return closest_coords
        except Exception as e:
            print(f"[SO2Extractor] Local CSV query failed: {e}")
