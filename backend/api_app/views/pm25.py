...
class PredictPM25View(APIView):
    ...
    def get(self, request):
        ...
        result = pm25_predictor.predict_at_coords(lat, lon, range_str, overrides=overrides)
        if result.get('error'):
...
class PredictPM25AtCoordsView(APIView):
    permission_classes = [permissions.IsAuthenticated]
...
class MapDataPM25View(APIView):
    ...
    def get(self, request):
        ...
        for name, info in TOWN_COORDS.items():
            ...
            result = pm25_predictor.predict_at_coords(lat, lon, '1Y')
            if result.get('error'):
                data.append({
