...
class PM25HuggingFaceAPI:
    def __init__(self, hf_token=None):
        """
        Initializes the connection to Hugging Face using DuckDB.
        """
        self.base_path = "hf://datasets/ObitUchiha91/Airlytics_data_set/PM25"
        ...
        try:
            ...
            print(f"[PM25Extractor] Failed to install/load httpfs extension: {e}")
        ...
        if token:
            try:
                ...
                print("[PM25Extractor] Successfully authenticated with Hugging Face.")
            except Exception as e:
                print(f"[PM25Extractor] Failed to create Hugging Face secret: {e}")
        else:
            print("[PM25Extractor] No HF_TOKEN provided. Dataset is public, so DuckDB should still work.")
    ...
    def get_state_summary(self, year="*"):
        ...
        query = f"""
            SELECT 
                ...
                AVG(pm25_level) as mean_pm25,
                MAX(pm25_level) as max_pm25
            FROM '{file_path}'
    ...
    def get_data_for_coordinate(self, lat, lon, year="2023"):
        ...
        if use_hf:
            try:
                ...
                if df.empty:
                    print(f"[PM25Extractor] Tight bbox empty, widening to 0.2 for ({lat}, {lon})")
                    df = self.get_by_bounding_box(lat - 0.2, lat + 0.2, lon - 0.2, lon + 0.2, year)
                
                if not df.empty:
                    print(f"[PM25Extractor] HF data: {len(df)} rows, columns: {list(df.columns)}")
                    ...
                    print(f"[PM25Extractor] âœ… SOURCE=HuggingFace for ({lat}, {lon}), "
                          f"closest={len(closest_coords)} rows, dist={min_dist:.6f}")
                    return closest_coords
                else:
                    print(f"[PM25Extractor] âš  HF returned empty for ({lat}, {lon}), year={year}")
            except Exception as e:
                print(f"[PM25Extractor] âŒ Hugging Face query failed: {e}. Falling back to local CSV.")
        ...
        try:
            ...
            if year == "*":
                file_pattern = os.path.join(csv_dir, "PM25_Data_Improv*.csv")
            else:
                file_pattern = os.path.join(csv_dir, f"PM25_Data_Improv{year}.csv")
            
            if not os.path.exists(csv_dir):
                print(f"[PM25Extractor] Local CSV directory not found at {csv_dir}")
                return pd.DataFrame()
            ...
            if not df.empty:
                ...
                closest_coords = df[df['dist'] == min_dist]
                closest_coords = closest_coords.drop(columns=['dist'])
                print(f"[PM25Extractor] Loaded closest coordinates from local CSV fallback for ({lat}, {lon})")
                return closest_coords
        except Exception as e:
            print(f"[PM25Extractor] Local CSV query failed: {e}")
